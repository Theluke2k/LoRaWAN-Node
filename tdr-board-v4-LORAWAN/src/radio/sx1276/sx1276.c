...
static void RxChainCalibration( void )
{
	//printf("RxChainCalibration\n");
    uint8_t regPaConfigInitVal;
    uint32_t initialFreq;

    //printf("RxChainCalibration: 1\n");
    // Save context
    regPaConfigInitVal = SX1276Read( REG_PACONFIG );
    //printf("RxChainCalibration: 2\n");
    initialFreq = SX1276ConvertPllStepToFreqInHz( ( ( ( uint32_t )SX1276Read( REG_FRFMSB ) << 16 ) |
                                                    ( ( uint32_t )SX1276Read( REG_FRFMID ) << 8 ) |
                                                    ( ( uint32_t )SX1276Read( REG_FRFLSB ) ) ) );
    //printf("RxChainCalibration: 3\n");
    // Cut the PA just in case, RFO output, power = -1 dBm
    SX1276Write( REG_PACONFIG, 0x00 );
    //printf("RxChainCalibration: 4\n");
    // Launch Rx chain calibration for LF band
    SX1276Write( REG_IMAGECAL, ( SX1276Read( REG_IMAGECAL ) & RF_IMAGECAL_IMAGECAL_MASK ) | RF_IMAGECAL_IMAGECAL_START );
    //printf("RxChainCalibration: 5\n");
    while( ( SX1276Read( REG_IMAGECAL ) & RF_IMAGECAL_IMAGECAL_RUNNING ) == RF_IMAGECAL_IMAGECAL_RUNNING )
    {
    }

    //printf("RxChainCalibration: 6\n");
...
/*
 * Lucas:
 * The function is used to read data from the sx1276 radio.
 * Here's how it works:
 * 1. Set NSS to 0 to initiate a the transmission.
 * 2. Transmit the address of the register that we want to read from.
 *    The MSB is set to 0 to indicate a read operation followed by 7 address bits.
 *    This opeartion returns a dummy byte. The data comes in next SPI clock cycle.
 * 3. We loop for each byte to receive from SPI. Each byte is saved in a buffer array.
 *    While we receive data, we send a dummy byte of 0x00. This must be done to
 *    initiate an SPI clock cycle.
 * 4. We set NSS to high to indicate the end of the transmission.
 *
 *
 * UPDATE: This function has to be rewritten to work with thebuilt in function from adi
 * to read SPI data.
 */
...
static void SX1276OnTimeoutIrq( void* context )
{
    ...
    case RF_RX_RUNNING:
        ...
        break;
    case RF_TX_RUNNING:
    	//printf("SX1276OnTimeoutIrq: RF_TX_RUNNING\n");
